...
def create_can_steer_command(packer, steer_angle, steer_req, is_standstill, raw_cnt):
  """50 Hz, spoofed ADAS camera steering request."""
  set_me_xe = 0xB
  if is_standstill:
    set_me_xe = 0xE
...
def create_accel_command(packer, accel, enabled, brake_hold, raw_cnt):
  """ACC_CMD spoof for openpilot longitudinal control. Not in use, needs real vehicle validation."""
  accel = min(max(accel * 16.67, -50.), 30.)
...
def create_lkas_hud(packer, enabled, lss_state, lss_alert, tsr, ahb, passthrough, hma, pt2, pt3, pt4, pt5, lka_on, raw_cnt):
  ...
  dat = packer.make_can_msg("LKAS_HUD_ADAS", 0, values)[1]
  crc = byd_checksum(0xAF, dat[:-1])
  values["CHECKSUM"] = crc
...
def send_buttons(packer, state, count):
  """Spoof ACC SET/RES button press for auto-resume from standstill."""
  values = {
    "SET_BTN": state,
    "RES_BTN": state,
    "SET_ME_1_1": 1,
